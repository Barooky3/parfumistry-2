...
// Mirrors the real Rewarble.tsx page, then after confirming switches to the actual
// /checkout?completed=rewarble success card from Checkout.tsx.
export const ScenePaste: React.FC = () => {
  ...
  // Code input center ≈ (800, 360). Confirm button center ≈ (800, 580). After click → success screen at frame 120.
  const inputX = 800, inputY = 360;
  const confirmX = 800, confirmY = 580;
  const cx = interpolate(frame, [0, 30, 90, 130], [1100, inputX - 2, inputX - 2, confirmX - 2], { extrapolateRight: "clamp" });
  const cy = interpolate(frame, [0, 30, 90, 130], [500, inputY - 2, inputY - 2, confirmY - 2], { extrapolateRight: "clamp" });
  const clickingConfirm = frame >= 100 && frame <= 115;
  const success = frame >= 118;
      ...
      <BrowserChrome url={success ? "parfumistry.net/checkout" : "parfumistry.net/rewarble"}>
        ...
        {clickingConfirm && (
          <div style={{
            ...
            width: 36, height: 36, borderRadius: 18,
            border: "3px solid #16a34a", opacity: Math.max(0, 1 - (frame - 100) / 15),
            transform: `scale(${1 + (frame - 100) / 12})`,
            pointerEvents: "none",
          }} />
        )}
      </BrowserChrome>
